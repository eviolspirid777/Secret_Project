...
io.on('connection', async (socket) => {
  ...
  socket.on('create-transport', async ({ roomId }, callback) => {
    try {
      const transportOptions = await mediaServer.createWebRtcTransport(roomId, socket.id);
  ...
  socket.on('connect-transport', async ({ roomId, transportId, dtlsParameters }, callback) => {
    try {
      await mediaServer.connectTransport(roomId, socket.id, transportId, dtlsParameters);
  ...
  socket.on('produce', async ({ roomId, transportId, kind, rtpParameters }, callback) => {
    try {
      const producer = await mediaServer.createProducer(roomId, socket.id, transportId, kind, rtpParameters);
  ...
  socket.on('consume', async ({ roomId, producerId, rtpCapabilities }, callback) => {
    try {
      const consumer = await mediaServer.createConsumer(roomId, socket.id, producerId, rtpCapabilities);
      callback({ consumer });
    } catch (error) {
